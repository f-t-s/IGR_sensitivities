#!/usr/bin/env julia
# ============================================================
# Generate heatmap images for the Oseen-pair / blast figure.
#
# A Gaussian over-pressure blast (A = 12) sweeps over a counter-
# rotating pair of physical Oseen vortices (Colonius, Lele & Moin,
# JFM 1991; see build_oseen_pair_blast_ic!). Same problem as the
# OtD refinement study in generate_otd_refinement_oseen_data.jl.
#
...
# then framed by a pgfplots axis in figures/tikz/oseen_heatmaps.tex
# (which supplies the frame, ticks, axis labels, and colorbar).
#
# Five panels, in the layout used by the figure:
#   primal  (forward at T):     u_x, u_y, Σ
#   adjoint (PDE adjoint at 0): a†_ux, a†_uy
# The adjoint is for the windowed-kinetic-energy objective
# (:windowed_kinetic_energy, window KE_WINDOW).
#
# Outputs (in figures/data/):
#   oseen_heatmap_<panel>.png   — bare rasterized field, one per panel
#   oseen_heatmap_defs.tex      — generated pgfplots colormap definitions
#                                 and per-panel \point-meta option macros
# ============================================================
...
# ============================================================
# Problem setup — same problem as the Oseen OtD refinement study.
# ============================================================
γ   = 1.4
L   = 1.0
p   = 4
N_e = 160
α   = 3.0 * (L / 20)^2   # FIXED physical IGR regularization
T   = 0.10
CFL = 0.4
n_iter = 60              # PCG iterations — converged elliptic solves
solver = :pcg
...
@printf("Oseen heatmap data: objective=%s, T=%.3f\n", objective, T)
@printf("  p=%d, N_e=%d×%d, α=%.4e, A=%.1f, n_iter=%d, solver=%s\n",
        p, N_e, N_e, α, blast_A, n_iter, solver)
@printf("  window: xc=%.2f, yc=%.2f, σ=%.2f\n", KE_WINDOW.xc, KE_WINDOW.yc, KE_WINDOW.σ)

# ============================================================
# Oseen-pair + blast initial condition
# ============================================================
μx0 = zeros(size(mesh.x)); μy0 = zeros(size(mesh.x))
ρ0  = similar(mesh.x);     E0  = similar(mesh.x)
build_oseen_pair_blast_ic!(μx0, μy0, ρ0, E0, blast_A, mesh, γ;
...
for (key, Q, cmap, pgfname, diverging) in panels
    ...
    ranges[key]  = cr
    mapname[key] = pgfname
    png = joinpath(DATA_DIR, "oseen_heatmap_$(key).png")
...
defs = joinpath(DATA_DIR, "oseen_heatmap_defs.tex")
open(defs, "w") do io
    println(io, "% GENERATED by paper_scripts/generate_oseen_heatmap_data.jl — do not edit by hand.")
    ...
    for (key, _, _, _, _) in panels
        ...
        @printf(io, "\\def\\oseenOpts%s{colormap name=%s, point meta min=%.6e, point meta max=%.6e}\n",
                MACRO[key], mapname[key], cmin, cmax)
    end
end

println("\nWrote:")
println("  8 PNGs           → $DATA_DIR/oseen_heatmap_*.png")
println("  colormap+limits  → $defs")
println("Figure layout: figures/tikz/oseen_heatmaps.tex (\\input's the defs).")
